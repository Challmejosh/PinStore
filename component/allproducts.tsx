...
interface Prop{
    products: FetchData[]|null
}
const AllProducts = ({products}: Prop) => {
    ...
    return ( 
        <div className="flex flex-col md:bg-transparent gap-2">
                <div className="overflow-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 lg:grid-cols-6">
                    {products?.map((item: FetchData)=>(
                        <div key={item?.id} className={` overflow-auto relative h-full border-[#9CA3AF] border-l border flex p-2 flex-col items-center justify-between `}>
                            ...
                            <div className="flex w-full items-center justify-end ">
                                <div onClick={()=>handleCart(products,item)} className="shadow-lg p-2 cursor-pointer flex items-center justify-center bg-red-200 rounded-full w-fit h-fit overflow-hidden ">
