                ...
                {!pending && 
                    <>
                        {data?.slice(0,9).map((item: Items,index)=>(
                            <div key={index} className={`${index === 0 && "rounded-l-[10px] "} ${index === 8 && "rounded-r-[10px] border-r "} border-[#9CA3AF] border-l cursor-pointer border-y flex p-2 flex-col items-center justify-between h-[150px] `}>
            ...
            <div className="lg:hidden w-full">
                ...
                className="w-full ">
                    {!pending && 
                        <>
                            {data?.slice(0,9).map((item: Items,index)=>(
                                <SwiperSlide key={index} className={` border-[#9CA3AF] border cursor-pointer border-y flex p-2 flex-col items-center justify-between h-[150px] `}>
                                    <p className="w-full h-[120px]  ">
                                        <img src={item?.url} alt={item?.slug} className="object-contain" />
                                    </p>
                                    <p className="text-xs ">{item?.name}</p>
